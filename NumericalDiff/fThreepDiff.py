...
def forward_three_point_differentiation(f, x, h=1e-5):
    """
    Approximate the derivative of a function at a point using forward (progressive) three-point differentiation.
...
if __name__ == "__main__":
    ...
    # Perform forward three-point differentiation and store intermediate results
    results = []
    x0 = a
    while x0 <= b:
        estimated_derivative = forward_three_point_differentiation(user_function, x0, step_size)
        results.append((x0, estimated_derivative))
        x0 += step_size

    # Print the table of derivatives within the interval
    print("\nDerivatives within the Interval (Forward Three-Point Differentiation):")
